...
│       │   ├── session.py               # AgentSession, AgentDivisionPlan
...
- **`uuid: UUID`** — canonical identity for the session. Generated by the agent (uuid4) on first write, never changes. The Pydantic model has `uuid: UUID = Field(default_factory=uuid.uuid4)`. The `id` (slug, e.g. `api-endpoints-core`) is the human-readable label. See the "ID Allocation — Dual UUID + Sequential Keys" section.
...
```
sessions/
├── api-endpoints-core.yaml          # session definition + runtime state + engagements
└── api-endpoints-core/
...
```markdown
# Recommended Testing Plan — api-endpoints-core
...
```markdown
# Post-Completion Comments — api-endpoints-core
