"""Tests for the self-review JIT prompt — fires on session.complete."""
...
from tripwire._internal.jit_prompts.self_review import _VARIATIONS, SelfReviewJitPrompt
...
def test_self_review_fires_returns_one_of_the_variations(tmp_path: Path) -> None:
    tw = SelfReviewJitPrompt()
    ctx = JitPromptContext(project_dir=tmp_path, session_id="alpha", project_id="proj")
    prompt = tw.fire(ctx)
