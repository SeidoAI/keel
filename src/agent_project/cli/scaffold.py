"""`agent-project scaffold-for-creation` — front-load the agent's context.
...
from agent_project.core.id_generator import format_key
from agent_project.core.store import ProjectNotFoundError, load_project
...
def collect_scaffold(project_dir: Path) -> ScaffoldData:
    ...
    return ScaffoldData(
        ...
        validation_gate={
            "command": "agent-project validate --strict --format=json",
            "exit_codes": {
                "0": "clean",
                "1": "warnings only",
                "2": "errors",
        ...
        id_allocation={
            "sequential_keys": "agent-project next-key --type issue",
...
def _render_text(data: ScaffoldData) -> str:
    ...
    section of `docs/agent-projects-plan.md` — the same output an agent reads
...
@click.command(name="scaffold-for-creation")
@click.option(
    "--project-dir",
...
def scaffold_cmd(project_dir: Path, output_format: str) -> None:
    ...
    command, and ID allocation rules. The PM skill instructs the agent to
    run this first, before reading any planning docs or drafting files.
    """
