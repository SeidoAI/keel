...
`cProfile` output file. Defaults to `.agent-project.profile` in the cwd
...
DEFAULT_PROFILE_PATH = ".agent-project.profile"
...
def profileable(command: Callable[..., Any]) -> Callable[..., Any]:
    ...
    `.agent-project.profile`. When passed with a value (`--profile=foo.prof`),
