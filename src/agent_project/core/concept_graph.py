...
this, and the CLI `agent-project graph --type concept` uses it too.
"""
...
from agent_project.core.store import list_issues
...
def _from_scan(project_dir: Path) -> FullGraphResult:
    ...
    from agent_project.core.graph_cache import _issue_edges, _node_edges
...
def _compute_orphans(all_ids: set[str], edges: list[GraphEdge]) -> list[str]:
    ...
    list them with `agent-project refs check`.
    """
