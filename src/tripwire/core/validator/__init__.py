...
def _load_nodes(ctx: ValidationContext) -> None:
    ...
    for path in sorted(nodes_root.glob("*.yaml")):
        # `nodes/tripwire-graph-index.yaml` is the derived graph cache
        # that lives alongside source nodes since v0.10.0; skip it
        # silently (it's regenerated by the graph-cache module, not
        # authored as a concept node). The id is also rejected as a
        # node id elsewhere so a user can't accidentally write one.
